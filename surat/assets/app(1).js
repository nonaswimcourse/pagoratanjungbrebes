...
// ============ ALAT SEKALI-PAKAI: BUAT FOLDER DRIVE BARU LEWAT APP ============
// Kenapa perlu ini: scope OAuth yang dipakai ('drive.file') cuma boleh menulis ke
// file/folder yang DIBUAT LEWAT APP INI (atau dipilih user lewat Google Picker).
// Kalau GOOGLE_DRIVE_FOLDER_ID di config.js menunjuk ke folder yang dibuat manual
// di drive.google.com biasa, upload akan selalu gagal 403
// "Insufficient permissions for the specified parent" walau folder itu milik
// akun yang sama. Jalankan fungsi ini SEKALI dari console browser (F12) di
// halaman /surat untuk membuat folder baru yang otomatis "dimiliki" oleh app,
// lalu salin folder ID yang muncul di console ke GOOGLE_DRIVE_FOLDER_ID.
//
// Cara pakai di console:
//   await createDriveFolderSekaliPakai('Surat KKG PJOK')
window.createDriveFolderSekaliPakai = async function (namaFolder) {
  const nama = namaFolder || 'Surat SIAP Tanjung';
  console.log('Meminta izin Google...');
  const accessToken = await requestGoogleAccessToken();
  console.log('Membuat folder "' + nama + '" di Drive...');
  ...
  if (!res.ok) {
    const errText = await res.text().catch(() => '');
    console.error('Gagal bikin folder:', res.status, errText);
    throw new Error('Gagal bikin folder: HTTP ' + res.status);
  }
  const data = await res.json();
  console.log('%cBERHASIL! Folder ID baru: ' + data.id, 'font-size:16px;font-weight:bold;color:green');
  console.log('Link folder:', data.webViewLink);
  console.log('Salin ID ini ("' + data.id + '") ke GOOGLE_DRIVE_FOLDER_ID di surat/config.js, lalu ganti isi folder lama ke folder baru ini kalau perlu.');
  return data;
};
